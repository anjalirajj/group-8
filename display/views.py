    ...
    if request.method == 'POST':
        ...
        if(registerd):
            ...
            if not profile_obj.is_verified:
                error_msg = "Email has not been verified,check your email"
                context = {"error_condition": error_msg}
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        elif all(c.isalpha() == first_isalpha for c in password):
            error_msg ="Password must contain at least one letter and at least one special character." 
            context = {"username_error": error_msg}
            return render(request, "signup.html", context)                            
        else:
